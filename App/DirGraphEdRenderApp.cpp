#include "DirGraphEdRenderApp.hpp"

DirGraphEdRenderApp::DirGraphEdRenderApp(HWND screen, UINT width, UINT height):
...

{
  ...
  mShaderGraphLines = NULL;

  mHasModel = false;

}

DirGraphEdRenderApp::~DirGraphEdRenderApp()
{  
...
bool DirGraphEdRenderApp::init()
{
  if (!BaseDX::init())
    ...
    mShaderGraphLines  = new Shader();
    mShaderGraphLines->Init(gStoredShaders[shadFwdLineGraphPC], md3dDevice );
...
void DirGraphEdRenderApp::drawScene (
    D3DXVECTOR3& camPos,
    D3DXVECTOR3& camTar,
    D3DXVECTOR3& camUp,
    GridModel* aGrid,
    ...
    )
{
  ...
  mShaderGraphLines->SetMatrix( &(std::string("invView")), &inv );
  mShaderGraphLines->SetMatrix( &(std::string("World")), &mWorld );
  mShaderGraphLines->SetMatrix( &(std::string("View")), &mView );
  mShaderGraphLines->SetMatrix( &(std::string("Projection")), &mProjection );

  md3dDevice->IASetInputLayout( mShaderGraphLines->mVertexLayout );
  md3dDevice->IASetPrimitiveTopology( D3D10_PRIMITIVE_TOPOLOGY_LINELIST );

  pass = mShaderGraphLines->mTech->GetPassByIndex(0);
  pass->Apply(0);
...
void  DirGraphEdRenderApp::setModel( EditableDirGraph* aModel  )
{
  if (mHasModel)
    delete mModel;
  mModel = aModel;
...
ID3D10Device* DirGraphEdRenderApp::getD3DXdevice()
{
  return md3dDevice;
}
