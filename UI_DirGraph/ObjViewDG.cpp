...
ObjViewDG* ObjViewDG::ViewFree()
{
  std::wstring title(L"Free");
  return new ObjViewDG( title , eTypeFree,
                      true,
...
D3DXVECTOR3  ObjViewDG::projectPoint( float aX, float aY )
{
  D3DXVECTOR3 ret;
...
D3DXVECTOR3  ObjViewDG::projectPoint( D3DXVECTOR2& aP )
{
  D3DXVECTOR3 ret;
...
D3DXVECTOR3 ObjViewDG::getCamAt()
{
  return projectPoint( mCamPan );
}

D3DXVECTOR3 ObjViewDG::getCamPos()
{
  D3DXVECTOR3 ret;
...
void ObjViewDG::panUpdate(float aX, float aY)
{
...
float ObjViewDG::scaleUpdate (float aScale )
{
...
void ObjViewDG::orbitUpdate(float aX, float aY)
{
...
bool ObjViewDG::allowsOrbitation()
{
  return mAllowOrbitation;
}

D3DXVECTOR3 ObjViewDG::getTangentVectorU()
{
  return mUnitTangentU;
}

D3DXVECTOR3 ObjViewDG::getTangentVectorV()
{
  return mUnitTangentV;
}

float ObjViewDG::getScale()
{
  return mCamScale;
}

D3DXVECTOR3 ObjViewDG::getUpVector()
{
  return mRenderUpVector;
}

bool ObjViewDG::isFreeView()
{
  return (mTypeView == 3);
}

std::wstring ObjViewDG::getTitle()
{
  return mTitle;
}

float ObjViewDG::getGridScale()
{
...
void ObjViewDG::setGridScaleMultiplier(float aMult )
{
  mGridScaleMult = aMult;
}


D3DXVECTOR3  ObjViewDG::adjustPointToGrid( const D3DXVECTOR3& aPoint )
{
  D3DXVECTOR3 ret;
...
float ObjViewDG::adjustCoordinate (const float aCoord, float aMod )
{
...
*/
D3DXVECTOR3 ObjViewDG::unitVector()
{
  D3DXVECTOR3 ret;
  if (mTypeView != 3)
    ret  = mUnitVector;
...
D3DXVECTOR3 ObjViewDG::cameraTangetU()
{
  float r = mCamScale;
  float t = mCamOrbit.x;
  float p = mCamOrbit.y;
...
D3DXVECTOR3 ObjViewDG::cameraTangetV()
{
  float r = mCamScale;
  float t = mCamOrbit.x;
  float p = mCamOrbit.y;
...
D3DXVECTOR3 ObjViewDG::getCamTangentU()
{
  float thita = mCamOrbit.y;
  float rho   = mCamOrbit.x;
...
D3DXVECTOR3 ObjViewDG::getCamTangentV()
{
  float thita = mCamOrbit.y;
  float rho   = mCamOrbit.x;
